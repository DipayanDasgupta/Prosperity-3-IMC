...
class Logger:
    # (Keep the full Logger class implementation from the previous correct responses)
...
class Strategy:
    # (Keep implementation as before, with corrected run method)
...
class SquidInkStrategyV13_RelaxedBB(Strategy): # Renamed from V10
    """Relaxed Bollinger Bands mean reversion for SQUID_INK."""
    def __init__(self, symbol: Symbol, limit: int) -> None:
        super().__init__(symbol, limit)
        self.bb_window_size = 20
        self.bb_std_dev_multiplier = 2.0
        self.price_history = deque(maxlen=self.bb_window_size)
        ...
        self.passive_order_size = 4    # Passive size
        self.aggressive_order_size = 2 # Aggressive take size
        self.entry_offset = 1          # Place passive orders 1 tick inside bands

    def calculate_bollinger_bands(self) -> None:
        if len(self.price_history) < self.bb_window_size:
            self.sma=None; self.std_dev=None; self.upper_band=None; self.lower_band=None; return
        prices = list(self.price_history); self.sma = np.mean(prices); self.std_dev = np.std(prices)
        min_std_dev = 0.5; self.std_dev = max(self.std_dev, min_std_dev)
        self.upper_band = self.sma + self.bb_std_dev_multiplier * self.std_dev
        self.lower_band = self.sma - self.bb_std_dev_multiplier * self.std_dev

    def get_current_midprice(self, state: TradingState) -> Optional[float]:
        # (Same midprice logic as before)
        ods=state.order_depths.get(self.symbol);
        if not ods: return None
        bids=ods.buy_orders; asks=ods.sell_orders;
        if not bids or not asks: return min(asks.keys()) if asks else (max(bids.keys()) if bids else None)
    ...
    def act(self, state: TradingState) -> None:
        ...
        if self.lower_band is None or self.upper_band is None: return # Need bands to trade

        # Fair value proxy is SMA, but we use bands directly for placement now
        # fair_value = int(round(self.sma)) if self.sma is not None else None

        order_depth=state.order_depths[self.symbol]; buy_orders=sorted(order_depth.buy_orders.items(),reverse=True); sell_orders=sorted(order_depth.sell_orders.items())
        pos=state.position.get(self.symbol,0); buy_cap=self.limit-pos; sell_cap=self.limit+pos
        ...
        # --- Aggressive Mean Reversion Orders ---
        # Buy aggressively if price *crosses significantly below* lower band
        if best_ask < self.lower_band - 1 and buy_cap > 0: # Require crossing by 1 tick
            vol=min(buy_cap,abs(order_depth.sell_orders[best_ask]),self.aggressive_order_size); self.buy(best_ask,vol); buy_cap-=vol
            # logger.print(f"SQUID BB Aggressive BUY: {vol}@{best_ask} (LowBand: {self.lower_band:.1f})")

        # Sell aggressively if price *crosses significantly above* upper band
        if best_bid > self.upper_band + 1 and sell_cap > 0: # Require crossing by 1 tick
            vol=min(sell_cap,order_depth.buy_orders[best_bid],self.aggressive_order_size); self.sell(best_bid,vol); sell_cap-=vol
            # logger.print(f"SQUID BB Aggressive SELL: {vol}@{best_bid} (HighBand: {self.upper_band:.1f})")


        # --- Passive Mean Reversion Orders (Relaxed) ---
        # Place passive BUY order *near* the lower band
        if buy_cap > 0:
             target_buy_price = int(round(self.lower_band + self.entry_offset))
             # Simple placement - place the order regardless of current best bid/ask (unless it implies crossing)
             # Ensure we don't place higher than the best ask
             if best_ask < float('inf'):
                 target_buy_price = min(target_buy_price, best_ask -1)

             self.buy(target_buy_price, min(buy_cap, self.passive_order_size))
             # logger.print(f"SQUID BB Passive BUY: {min(buy_cap, self.passive_order_size)}@{target_buy_price}")

        # Place passive SELL order *near* the upper band
        if sell_cap > 0:
             target_sell_price = int(round(self.upper_band - self.entry_offset))
             # Ensure we don't place lower than the best bid
             if best_bid > float('-inf'):
                 target_sell_price = max(target_sell_price, best_bid + 1)

             self.sell(target_sell_price, min(sell_cap, self.passive_order_size))
             # logger.print(f"SQUID BB Passive SELL: {min(sell_cap, self.passive_order_size)}@{target_sell_price}")

    # Save/Load Bollinger state
    def save(self) -> JSON: return {"price_history": list(self.price_history)} # Only need history
    def load(self, data: JSON) -> None:
        if isinstance(data,dict): history = data.get("price_history",[]); self.price_history = deque(history, maxlen=self.bb_window_size); self.calculate_bollinger_bands() # Recalc bands on load
...
class Trader:
    def __init__(self) -> None:
        ...
        self.strategy_map = {
            ...
            "KELP": KelpStrategy,
            "SQUID_INK": SquidInkStrategyV13_RelaxedBB, # Use the relaxed BB strategy
        }
