...
from typing import Any, TypeAlias, List, Dict, Optional, Tuple # Make sure all are imported
...
class Strategy:
    ...
    # Corrected signature with full return type hint
    def run(self, state: TradingState, context: Optional[Dict]=None) -> List[Order]:
        self.orders = []
        try:
            self.act(state, context) # Pass context to act
    ...
    @abstractmethod
    def act(self, state: TradingState, context: Optional[Dict]=None) -> None: # Ensure act accepts context
...
class MarketMakingStrategy(Strategy):
    # (Keep implementation as before, including EMA logic and save/load)
    ...
    def act(self, state: TradingState, context: Optional[Dict]=None) -> None: # Default MM logic, accept context
        ...
        pass_size=5 # Default passive size
...
class RainforestResinStrategy(Strategy):
    # (Keep implementation as before, accepting context)
    def __init__(self, symbol: Symbol, limit: int) -> None: super().__init__(symbol, limit); self.fair_value = 10000
    def act(self, state: TradingState, context: Optional[Dict]=None) -> None:
        ...
        squid_trend=context.get('squid_trend',0) if context else 0; passive_size=4 if squid_trend==-1 else 5; taken=False
...
class KelpStrategy(MarketMakingStrategy):
    ...
    def act(self, state: TradingState, context: Optional[Dict]=None) -> None:
        ...
        fair_value=int(round(fair_value_float)); ods=state.order_depths[self.symbol];buy_orders=sorted(ods.buy_orders.items(),reverse=True);sell_orders=sorted(ods.sell_orders.items())
        pos=state.position.get(self.symbol,0); buy_cap=self.limit-pos; sell_cap=self.limit+pos
        squid_trend=context.get('squid_trend',0) if context else 0; passive_spread_adj=1 if squid_trend==-1 else 0
        pass_buy_p=fair_value-(1+passive_spread_adj); pass_sell_p=fair_value+(1+passive_spread_adj); aggr_buy_thr=fair_value; aggr_sell_thr=fair_value
        inv_frac=pos/self.limit
        if inv_frac>0.5: skew=int(round(inv_frac*2)); aggr_buy_thr-=skew; pass_buy_p-=skew # Correct variable name pass_buy_p
        elif inv_frac<-0.5: skew=int(round(abs(inv_frac)*2)); aggr_sell_thr+=skew; pass_sell_p+=skew # Correct variable name pass_sell_p
        taken_buy=0
        if sell_orders:
            for p,v_neg in sell_orders:
                if p<=aggr_buy_thr and buy_cap>0: v=abs(v_neg); vol=min(v,buy_cap); self.buy(p,vol); buy_cap-=vol; taken_buy+=vol
        ...
        passive_size=4 if squid_trend==-1 else 5
        if buy_cap>0 and taken_buy==0: self.buy(pass_buy_p, min(buy_cap, passive_size))
        if sell_cap>0 and taken_sell==0: self.sell(pass_sell_p, min(sell_cap, passive_size)) # Use correct variable pass_sell_p

class SquidInkStrategyV6_Reversed(MarketMakingStrategy):
    ...
    def __init__(self, symbol: Symbol, limit: int) -> None:
        ...
        self.passive_spread=1; self.passive_order_size=4; self.aggressive_order_size=3; self.trend_confirmation_threshold=0.5; self.calculated_trend=0
    def get_fair_value(self, state: TradingState) -> Optional[float]:
        current_mid=self.get_current_midprice(state); self.last_mid_price=current_mid
        if current_mid is not None:
            if self.ema_value is None: self.ema_value=current_mid
    ...
    def act(self, state: TradingState, context: Optional[Dict]=None) -> None:
        ...
        if fair_value_slow_ema is None or self.trend_ema_fast is None: self.calculated_trend=0; return
        fair_value=int(round(fair_value_slow_ema)); ods=state.order_depths[self.symbol];buy_orders=sorted(ods.buy_orders.items(),reverse=True);sell_orders=sorted(ods.sell_orders.items())
        pos=state.position.get(self.symbol,0); buy_cap=self.limit-pos; sell_cap=self.limit+pos
        aggr_buy_thr=fair_value; aggr_sell_thr=fair_value; inv_frac=pos/self.limit
        if inv_frac>0.5: skew=int(round(inv_frac*1.5)); aggr_buy_thr-=skew
        elif inv_frac<-0.5: skew=int(round(abs(inv_frac)*1.5)); aggr_sell_thr+=skew
        self.calculated_trend=0;
        if self.trend_ema_fast>fair_value_slow_ema+self.trend_confirmation_threshold: self.calculated_trend=1
        elif self.trend_ema_fast<fair_value_slow_ema-self.trend_confirmation_threshold: self.calculated_trend=-1
        taken_buy=0
        if sell_orders: best_ask,vol_neg=sell_orders[0];
        ...
        taken_sell=0
        if buy_orders: best_bid,vol_pos=buy_orders[0];
        ...
        if self.calculated_trend==1 and buy_cap>0: passive_buy_price=fair_value+self.passive_spread; self.buy(passive_buy_price, min(buy_cap, self.passive_order_size))
        elif self.calculated_trend==-1 and sell_cap>0: passive_sell_price=fair_value-self.passive_spread; self.sell(passive_sell_price, min(sell_cap, self.passive_order_size))
    def save(self) -> JSON: return {"ema_value":self.ema_value,"last_mid_price":self.last_mid_price,"trend_ema_fast":self.trend_ema_fast}
    def load(self, data: JSON) -> None: super().load(data); self.trend_ema_fast = data.get("trend_ema_fast", None) if isinstance(data, dict) else None; self.calculated_trend=0

# --- Main Trader Class ---
class Trader:
    # (Keep implementation as in the previous response, ensure it uses SquidInkStrategyV6_Reversed)
    def __init__(self) -> None:
        self.limits = { "RAINFOREST_RESIN": 50, "KELP": 50, "SQUID_INK": 50 }
        self.strategy_map = { "RAINFOREST_RESIN": RainforestResinStrategy, "KELP": KelpStrategy, "SQUID_INK": SquidInkStrategyV6_Reversed } # Use V6
        self.strategies: Dict[Symbol, Strategy] = {s: c(s, self.limits[s]) for s, c in self.strategy_map.items() if s in self.limits}
    ...
    def run(self, state: TradingState) -> Tuple[Dict[Symbol, List[Order]], int, str]:
        ...
        all_orders: Dict[Symbol, List[Order]] = {symbol: [] for symbol in self.limits}; conversions = 0; context = {}
        squid_strategy = self.strategies.get("SQUID_INK"); squid_trend_signal = 0
        if squid_strategy and "SQUID_INK" in state.order_depths:
            all_orders["SQUID_INK"] = squid_strategy.run(state, None) # Run Squid first
            if hasattr(squid_strategy, 'calculated_trend'): squid_trend_signal = squid_strategy.calculated_trend
        context['squid_trend'] = squid_trend_signal # Set context
        for symbol, strategy in self.strategies.items():
            if symbol == "SQUID_INK": continue # Skip re-running
            if symbol in state.order_depths: all_orders[symbol] = strategy.run(state, context) # Pass context
