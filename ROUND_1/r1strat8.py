...
class Strategy:
    # (Keep implementation as before - context removed)
...
class SquidInkStrategyV9_VolSpread(MarketMakingStrategy):
    """MM Strategy for SQUID_INK adjusting spread based on volatility."""
    def __init__(self, symbol: Symbol, limit: int) -> None:
        super().__init__(symbol, limit, ema_alpha=0.25) # Moderate EMA
        self.volatility_window_size = 20 # Look back window for volatility calc
        self.price_history = deque(maxlen=self.volatility_window_size)
        self.volatility: Optional[float] = None

        # Volatility/Spread Parameters (NEEDS TUNING)
        self.base_passive_spread = 1.5 # Minimum spread even in low vol
        self.volatility_spread_multiplier = 0.4 # How much volatility widens spread
        self.min_volatility = 1.0 # Floor for volatility calculation
        self.max_spread = 6.0 # Don't let spread get excessively wide
        ...
        # Inventory Skew
        self.aggr_skew_multiplier = 1.5 # How much inventory skews aggression
        self.passive_skew_multiplier = 0.5 # How much inventory skews passive price
    ...
    def get_fair_value(self, state: TradingState) -> Optional[float]:
        ...
        if len(self.price_history) >= self.volatility_window_size // 2: # Start calc with half window
            self.volatility = max(np.std(list(self.price_history)), self.min_volatility) # Use max with floor
        else:
            self.volatility = self.min_volatility # Use floor if not enough data

        return self.ema_value

    def act(self, state: TradingState) -> None:
        fair_value_float = self.get_fair_value(state) # Updates EMA and Volatility
        if fair_value_float is None or self.volatility is None:
            # logger.print(f"SQUID: Missing fair value or volatility")
            return
        ...
        target_half_spread = min(target_half_spread, self.max_spread / 2) # Cap the spread
        target_half_spread = max(target_half_spread, 1.0) # Ensure minimum spread of 2 (half-spread 1)
        # logger.print(f"SQUID: Vol={volatility:.2f}, TargetHalfSpread={target_half_spread:.2f}")
        ...
        inv_frac=pos/self.limit
        if inv_frac > 0.2: # Skew earlier and maybe less sharply than before
            skew_aggr = int(round(inv_frac * self.aggr_skew_multiplier))
            skew_pass = int(round(inv_frac * self.passive_skew_multiplier))
            aggr_buy_thr -= skew_aggr
            pass_buy_p -= skew_pass # Skew passive price too
        elif inv_frac < -0.2:
            skew_aggr = int(round(abs(inv_frac) * self.aggr_skew_multiplier))
            skew_pass = int(round(abs(inv_frac) * self.passive_skew_multiplier))
            aggr_sell_thr += skew_aggr
            pass_sell_p += skew_pass # Skew passive price too

        # Round passive prices after skew
        ...
        if sell_orders:
            ...
            if best_ask <= aggr_buy_thr and buy_cap > 0:
                vol=min(abs(vol_neg),buy_cap,self.aggressive_order_size); self.buy(best_ask,vol); buy_cap-=vol; taken_buy+=vol
        taken_sell=0
        if buy_orders:
            best_bid,vol_pos=buy_orders[0];
            if best_bid >= aggr_sell_thr and sell_cap > 0:
                vol=min(vol_pos,sell_cap,self.aggressive_order_size); self.sell(best_bid,vol); sell_cap-=vol; taken_sell+=vol

        # --- Passive Orders ---
        # Adjust size based on volatility regime (simple example)
        passive_size = self.passive_order_size_high_vol if volatility > (self.high_vol_threshold*0.8) else self.passive_order_size_base

        # Place orders if capacity allows (don't filter based on deviation/trend now)
        if buy_cap > 0 and taken_buy == 0: # Only place if no aggressive buy
        ...
        if sell_cap > 0 and taken_sell == 0: # Only place if no aggressive sell
    ...
    def save(self) -> JSON:
        base_state = super().save() # Get ema_value, last_mid_price
        base_state["price_history"] = list(self.price_history)
        base_state["volatility"] = self.volatility
        return base_state

    def load(self, data: JSON) -> None:
        super().load(data) # Load ema_value, last_mid_price
        if isinstance(data, dict):
             history = data.get("price_history", [])
             self.price_history = deque(history, maxlen=self.volatility_window_size)
             self.volatility = data.get("volatility", None)
             # Recalculate volatility if history is present but volatility isn't saved (optional robustness)
             if self.volatility is None and len(self.price_history) >= self.volatility_window_size // 2:
...
# --- Main Trader Class ---
class Trader:
    # (Keep implementation as before, but change the Squid Ink strategy)
    def __init__(self) -> None:
        self.limits = { "RAINFOREST_RESIN": 50, "KELP": 50, "SQUID_INK": 50 }
        self.strategy_map = {
            "RAINFOREST_RESIN": RainforestResinStrategy,
            "KELP": KelpStrategy,
            "SQUID_INK": SquidInkStrategyV9_VolSpread, # Use the new Vol Spread strategy
        }
