...
class MarketMakingStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        max_buy_price = true_value - 2 if position > self.limit * 0.4 else true_value - 1
        min_sell_price = true_value + 2 if position < -self.limit * 0.4 else true_value + 1
...
# -------------------------------
# Improved TrendVolatilityStrategy
# -------------------------------
...
class TrendVolatilityStrategy(MarketMakingStrategy):
    def __init__(self, symbol: Symbol, limit: int, analysis_params: Dict[str, Dict]) -> None:
        ...
        self.price_history = deque(maxlen=20)  # Track recent midprices
        self.trade_pnl = deque(maxlen=10)  # Track recent trade PnL
        self.drawdown_limit = -1000  # Pause trading if losses exceed this
        self.min_spread = 2  # Minimum bid-ask spread to trade
    ...
    def get_true_value(self, state: TradingState) -> int:
        ...
        # Compute smoothed value using weighted moving average
        if len(self.price_history) >= 5:
            weights = [1, 1.5, 2, 2.5, 3][:len(self.price_history)]
            weighted_sum = sum(p * w for p, w in zip(list(self.price_history)[-len(weights):], weights))
            weighted_avg = weighted_sum / sum(weights)
            true_value = (0.6 * weighted_avg + 0.4 * analysis_value)
        else:
        ...
        if len(self.price_history) >= 2:
            ...
            if price_range > 2 * volatility:
                return round(analysis_value)  # Fallback to analysis value if too volatile
    ...
    def act(self, state: TradingState) -> None:
        ...
        if best_ask - best_bid < self.min_spread:
            return  # Skip trading if spread is too tight

        # Check drawdown
        current_pnl = sum(self.trade_pnl)
        if current_pnl < self.drawdown_limit:
            return  # Pause trading if drawdown exceeded
        ...
        # Adjust position sizing based on recent PnL
        risk_factor = 1.0
        ...
        if to_buy > 0 and hard_liquidate:
            quantity = int((to_buy // 3) * risk_factor)
            if quantity > 0:
                self.buy(true_value - 1, quantity)
                to_buy -= quantity
                self.trade_pnl.append(0)  # Assume neutral PnL for liquidation
        ...
        if to_sell > 0 and hard_liquidate:
            quantity = int((to_sell // 3) * risk_factor)
            if quantity > 0:
                self.sell(true_value + 1, quantity)
                to_sell -= quantity
                self.trade_pnl.append(0)
...
# -------------------------------
# AdvancedMarketMakingStrategy (Unchanged)
# -------------------------------
...
class AdvancedMarketMakingStrategy(Strategy):
    ...
    def make_orders(self, state: TradingState, fair_value: float, position: int, buy_order_volume: int, sell_order_volume: int) -> tuple[List[Order], int, int]:
        ...
        bids_below = [price for price in order_depth.sell_orders.keys() if price < fair_value - disregard_edge]
