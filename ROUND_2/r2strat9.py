...
# -------------------------------
# TrendVolatilityStrategy with Stop-Loss
# -------------------------------
...
class TrendVolatilityStrategy(MarketMakingStrategy):
    def __init__(self, symbol: Symbol, limit: int, analysis_params: Dict[str, Dict]) -> None:
        ...
        # Set a stop-loss threshold to limit losses
        self.stop_loss_threshold = -3000  # Adjust this threshold as needed
        self.unrealized_pnl = 0

    def get_true_value(self, state: TradingState) -> int:
        params = self.analysis_params.get(self.symbol, {})
        base_value = params.get("median_price", 10000)
        # Cap the negative trend to limit aggressive downward adjustments
        trend = max(params.get("trend_slope", 0), -10)
    ...
    def act(self, state: TradingState) -> None:
        ...
        self.unrealized_pnl = self.estimate_unrealized_pnl(state)
        if self.unrealized_pnl < self.stop_loss_threshold:
            logger.print(f"[STOP-LOSS TRIGGERED] Symbol={self.symbol}, PnL={self.unrealized_pnl}")
            self.close_positions_at_market(state)
            return

        # Otherwise, proceed with the market-making strategy
        super().act(state)

    def estimate_unrealized_pnl(self, state: TradingState) -> float:
        # Naively estimate unrealized PnL as (current_mid - base_price) * position.
        position = state.position.get(self.symbol, 0)
        order_depth = state.order_depths.get(self.symbol)
        ...
        # Use median_price as a proxy for the fill price (in real systems, you would track your actual fills)
        fill_price = self.analysis_params[self.symbol].get("median_price", current_mid)
        return (current_mid - fill_price) * position

    def close_positions_at_market(self, state: TradingState):
        # Immediately flatten the current position by aggressively placing market orders.
        position = state.position.get(self.symbol, 0)
        order_depth = state.order_depths.get(self.symbol)
        if not order_depth:
            return
        if position > 0:
            # For a long position, sell at the best bid available.
            best_bid = max(order_depth.buy_orders.keys()) if order_depth.buy_orders else self.get_true_value(state)
            self.sell(best_bid, position)
        elif position < 0:
            # For a short position, buy at the best ask available.
            best_ask = min(order_depth.sell_orders.keys()) if order_depth.sell_orders else self.get_true_value(state)
            self.buy(best_ask, abs(position))

# -------------------------------
# AdvancedMarketMakingStrategy (for SQUID_INK and PICNIC_BASKET2)
# -------------------------------
...
class AdvancedMarketMakingStrategy(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        if self.symbol == "SQUID_INK":
            fair_value = self.dynamic_fair_value(state, traderObject)
        else:
...
# -------------------------------
# Trader Class with Reduced Limits
# -------------------------------

class Trader:
    def __init__(self) -> None:
        # Lowered limits to reduce exposure
        self.limits = {
            "RAINFOREST_RESIN": 20,
            "SQUID_INK": 20,
            "PICNIC_BASKET1": 20,
        ...
        self.params = {
            ...
            "PICNIC_BASKET2": {
                "fair_value": 100,
                "take_width": 2,          # widened spread
                "clear_width": 1,         # widened spread for clearing positions
                "disregard_edge": 2,
                "join_edge": 3,
                "default_edge": 5,
                "soft_position_limit": 30,  # lower soft limit
            },
        }
        self.strategies = {}
...
def simulate_trading():
    trader = Trader()
    trader_data = ""
    # Initialize positions with the new, lower limits
    ...
    for tick in range(5):
        ...
        for symbol in positions:
            # Price simulation logic (can be further refined)
