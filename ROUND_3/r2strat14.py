...
# -------------------------------
# Strategy from r2strat4.py (Enhanced)
# -------------------------------
...
class InkMeanReversionTrader(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        max_buy_qty = min(self.limit - position, self.limit // 4)
        max_sell_qty = min(self.limit + position, self.limit // 4)

        # Adjusted thresholds (less aggressive)
        if mid_price < mean - 1 * std_dev and position < self.limit:
            self.buy(best_ask, max_buy_qty)
        elif mid_price > mean + 1 * std_dev and position > -self.limit:
            self.sell(best_bid, max_sell_qty)
...
class AdvancedMarketMakingStrategy(Strategy):
    ...
    def take_best_orders(self, state: TradingState, fair_value: float, orders: List[Order], position: int) -> tuple[int, int]:
        ...
        if order_depth.sell_orders:
            ...
            if (not prevent_adverse or best_ask_qty <= adverse_volume) and best_ask <= fair_value - self.params["take_width"]:
                quantity = min(math.floor(best_ask_qty * risk_factor), self.limit - position, self.limit // 4)
                if quantity > 0:
        ...
        if order_depth.buy_orders:
            ...
            if (not prevent_adverse or best_bid_qty <= adverse_volume) and best_bid >= fair_value + self.params["take_width"]:
                quantity = min(math.floor(best_bid_qty * risk_factor), self.limit + position, self.limit // 4)
                if quantity > 0:
    ...
    def clear_position_order(self, state: TradingState, fair_value: float, orders: List[Order], position: int, buy_order_volume: int, sell_order_volume: int) -> tuple[int, int]:
        ...
        if position_after > 0:
            ...
            send_qty = min(math.floor(sell_qty * risk_factor), clear_vol, self.limit // 4)
            if send_qty > 0:
        ...
        if position_after < 0:
            ...
            send_qty = min(math.floor(buy_qty * risk_factor), clear_vol, self.limit // 4)
            if send_qty > 0:
...
# -------------------------------
# KelpStrategy (from r2strat11.py)
# -------------------------------
...
# -------------------------------
# Basket1Trader (from r2strat12.py for PICNIC_BASKET1)
# -------------------------------
...
class Basket1Trader(Strategy):
    ...
    def act(self, state: TradingState) -> None:
        ...
        # Cap trade size to 25% of limit
        max_buy_qty = min(self.limit - pos, self.limit // 4)
        max_sell_qty = min(self.limit + pos, self.limit // 4)

        if deviation < -threshold and pos < self.limit:
            self.buy(basket_best_bid, max_buy_qty)
        elif deviation > threshold and pos > -self.limit:
            self.sell(basket_best_ask, max_sell_qty)
...
class Trader:
    def __init__(self) -> None:
        self.limits = {
            "RAINFOREST_RESIN": 50,
            "SQUID_INK": 50,
            "PICNIC_BASKET1": 60,  # Slightly increased to capture more arbitrage
            "PICNIC_BASKET2": 100,
            "DJEMBES": 50,
            "KELP": 50,
            "CROISSANTS": 50,
        ...
        self.analysis_params = {
            ...
            "DJEMBES":         {"median_price": 13409, "trend_slope": -10, "volatility": 20},
            "KELP":            {"median_price": 2034,  "trend_slope": 0,   "volatility": 10},
            "CROISSANTS":      {"median_price": 4275,  "trend_slope": 0,   "volatility": 5},
            "JAMS":            {"median_price": 6542,  "trend_slope": 0,   "volatility": 5},
        }
        self.params = {
            "SQUID_INK": {
                "fair_value": 1836,  # Adjusted to match simulation
                "take_width": 1,
                "clear_width": 0.5,
            ...
            "PICNIC_BASKET2": {
                "fair_value": 30254,  # Adjusted to match simulation
                "take_width": 1,
                "clear_width": 0,
                "disregard_edge": 1,
                "join_edge": 2,
        ...
        self.strategies = {symbol: clazz(symbol, self.limits[symbol]) for symbol, clazz in {
            "RAINFOREST_RESIN": lambda symbol, limit: TrendVolatilityStrategy(symbol, limit, self.analysis_params),
            "KELP": KelpStrategy,  # Reverted to KelpStrategy
            "SQUID_INK": lambda symbol, limit: AdvancedMarketMakingStrategy(symbol, limit, self.params[symbol]),
            "PICNIC_BASKET1": Basket1Trader,
            "PICNIC_BASKET2": lambda symbol, limit: AdvancedMarketMakingStrategy(symbol, limit, self.params[symbol]),
            "CROISSANTS": lambda symbol, limit: TrendVolatilityStrategy(symbol, limit, self.analysis_params),
            "JAMS": lambda symbol, limit: TrendVolatilityStrategy(symbol, limit, self.analysis_params),
            "DJEMBES": lambda symbol, limit: TrendVolatilityStrategy(symbol, limit, self.analysis_params),
        }.items()}
