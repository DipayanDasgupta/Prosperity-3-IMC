...
"""
This code basically sucks. It doesn't make any profit and for some reason doesn't hedge properly
I have to go so I can't fix the issue why it is happening but will give another attempt tomorrow

We need to basically fit the m(t) and v(t) as a curve and trade the residuals of them 
Also try changing the threshold parameter in the vouchers as they are old values from an old
attempt so not correct at all.
"""
...
def update_voucher_regression(voucher_data):
    """
    Returns:
       coeffs: array [a, b, c] for the fitted quadratic curve
       base_iv: the fitted implied volatility at m_t = 0 (i.e., c)
       data_points: list of (m_t, v_t) pairs (optional, for diagnostic purposes)
    """
    m_vals = []
    v_vals = []
    ...
    for item in voucher_data:
        ...
        m_vals.append(m_t)
        # Compute v_t using BlackScholes (assuming your BlackScholes.implied_volatility is accessible)
        v_t = BlackScholes.implied_volatility(Vt, St, K, TTE)
        v_vals.append(v_t)
    
    # Fit quadratic: v = a*m^2 + b*m + c
    coeffs = np.polyfit(m_vals, v_vals, 2)
    base_iv = coeffs[2]  # The intercept c represents v_t(m_t = 0)
    return coeffs, base_iv, list(zip(m_vals, v_vals))
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Round 3 Begin integration of regression for VOLCANIC_ROCK vouchers
        ...
        mid_price_rock = self.get_voucher_midprice(state.order_depths[Product.VOLCANIC_ROCK], traderObject)
        
        for voucher in vouchers:
            if voucher in self.params and voucher in state.order_depths:
                mid_price_voucher = self.get_voucher_midprice(state.order_depths[voucher], traderObject.get(voucher, {}))
        ...
        for VOUCHER in vouchers:
            ...
            if VOUCHER in self.params and VOUCHER in state.order_depths:
                ...
                mid_price = self.get_voucher_midprice(state.order_depths[VOUCHER], traderObject.get(VOUCHER, {}))
                K_current = self.params[VOUCHER]["strike"]
                tte_current = self.params[VOUCHER]["starting_time_to_expiry"]
                m_t_current = np.log(K_current / mid_price) / np.sqrt(tte_current)
                v_t_current = BlackScholes.implied_volatility(mid_price, mid_price, K_current, tte_current)
                ...
                if "voucher_regression_data" in traderObject:
                    ...
                    if residual > threshold and rock_voucher_position > -self.PRODUCT_LIMIT[VOUCHER]:
                        # sell voucher
                        if state.order_depths[VOUCHER].buy_orders:
                            best_bid = max(state.order_depths[VOUCHER].buy_orders.keys())
                            quantity = min(abs(rock_voucher_position - (-self.PRODUCT_LIMIT[VOUCHER])), 10)
                            voucher_orders.append(Order(VOUCHER, best_bid, -quantity))
                    elif residual < -threshold and rock_voucher_position < self.PRODUCT_LIMIT[VOUCHER]:
                        # buy voucher
                        if state.order_depths[VOUCHER].sell_orders:
                            best_ask = min(state.order_depths[VOUCHER].sell_orders.keys())
                            quantity = min(abs(self.PRODUCT_LIMIT[VOUCHER] - rock_voucher_position), 10)
                ...
                if rock_order_depth is not None:
                    hedge_orders = self.rock_hedge_orders(
                        rock_order_depth,
