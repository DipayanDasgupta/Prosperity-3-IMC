...
# ParabolaFitIVStrategy class (updated from new voucher code)
class ParabolaFitIVStrategy:
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        # Apply custom threshold if strategy is for 10500
        if self.voucher == "VOLCANIC_ROCK_VOUCHER_10500":
            low_thresh = 0.02
            high_thresh = 0.01
        else:
            low_thresh = 0.019
        ...
        if v_t < current_fit - low_thresh and position < self.position_limit:
            qty = min(30, self.position_limit - position)
            result.append(Order(self.voucher, best_ask, qty))
        elif v_t > current_fit + high_thresh and position > -self.position_limit:
            qty = min(30, self.position_limit + position)
            result.append(Order(self.voucher, best_bid, -qty))
...
class Trader:
    def __init__(self, params=None):
        ...
        # Initialize VOLCANIC_ROCK specific attributes
