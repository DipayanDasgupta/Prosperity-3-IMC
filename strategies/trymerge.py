...
class Trader:
    ...
    def spam_orders(self, state : TradingState, product, signal_product):
        ...
        if self.signal[signal_product] == 2:
            ...
            for price, qty in orderdepth.sell_orders.items():
                if pos + abs(qty) > self.position_limits[product]:
                    break
                orders.append(Order(product, price, abs(qty)))
                pos += abs(qty)
            rem_buy = self.position_limits[product] - pos
            ...
            for price, qty in orderdepth.buy_orders.items():
                ...
                orders.append(Order(product, price, -abs(qty)))
                pos -= abs(qty)
            rem_sell = self.position_limits[product] + pos
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        prod = Product.VOLCANIC_ROCK
        self.update_signal(state, trader_state, prod)
        result[prod] = self.spam_orders(state, prod, prod)
